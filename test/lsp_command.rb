...
assert('lsp_goto_command') do
  ...
  app.ext.lsp['default'] = LSP::Client.new('ruby', {'args' => [File.dirname(__FILE__)+'/../misc/dummy_lsp.rb']})
  app.ext.lsp['default'].server_capabilities['hogehogeProvider'] = false
  ...
  app.ext.lsp['default'].server_capabilities['definitionProvider'] = true
  app.lsp_goto_command('definition', 'definitionProvider')
  assert_equal "[lsp] server is not running", app.logger.log[:debug].last

  app.ext.lsp['default'].start_server({})
  app.ext.lsp['default'].status = :running
  ...
  app.ext.lsp['default'].stop_server
end
